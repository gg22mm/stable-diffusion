...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------
    trainer_opt = trainer_config #argparse.Namespace(**trainer_config)   # {'benchmark': True, 'accumulate_grad_batches': 2}  - trainer_config = lightning_config.get("trainer", OmegaConf.create())    
    ...
    # model
    # model = instantiate_from_config(config.model) #.yaml自动方式
    ...
    # else:#执行这里
    #     logger_cfg = OmegaConf.create() #空对象:{}        

    # logger_cfg = OmegaConf.merge(default_logger_cfg, logger_cfg) #logger_cfg=testtube
    # trainer_kwargs["logger"] = instantiate_from_config(logger_cfg) #自动方式
    ...
    # else:#执行这里
    #     modelckpt_cfg = OmegaConf.create()#空对象:{}   
    ...
    # else:#执行这里
    #     callbacks_cfg = OmegaConf.create()#空对象:{}

    # if 'metrics_over_trainsteps_checkpoint' in callbacks_cfg: #不执行这里
    ...
    # callbacks_cfg = OmegaConf.merge(default_callbacks_cfg, callbacks_cfg) # callbacks_cfg = default_callbacks_cfg
 
    # # trainer_opt = trainer_config # {'benchmark': True, 'accumulate_grad_batches': 2}  - trainer_config = lightning_config.get("trainer", OmegaConf.create())
    ...
    trainer_kwargs["callbacks"] = [
        getattr( importlib.import_module('main', package=None) , 'SetupCallback' )( **default_callbacks_cfg.get('setup_callback').get("params", dict()) ),
        getattr( importlib.import_module('main', package=None) , 'ImageLogger' )( **default_callbacks_cfg.get('image_logger').get("params", dict()) ),
        getattr( importlib.import_module('main', package=None) , 'LearningRateMonitor' )( **default_callbacks_cfg.get('learning_rate_logger').get("params", dict()) ),
        getattr( importlib.import_module('main', package=None) , 'CUDACallback' )(),        
    ]
    ...
    # NOTE according to https://pytorch-lightning.readthedocs.io/en/latest/datamodules.html #注根据https://pytorch-lightning.readthedocs.io/en/latest/datamodules.html
    # calling these ourselves should not be necessary but it is.  我们自己称之为这些不应该是必要的，但事实确实如此。
    # lightning still takes care of proper multiprocessing though 尽管如此，闪电仍然负责适当的多处理
    print('----------end1111---------',data)    
    data.prepare_data()
    data.setup()
    ...
    if 'accumulate_grad_batches' in lightning_config.trainer:
        accumulate_grad_batches = lightning_config.trainer.accumulate_grad_batches #2 执行的是这里
    else:
    ...
    if opt.scale_lr: #执行这里
        model.learning_rate = accumulate_grad_batches * ngpu * bs * base_lr # accumulate_grad_batches2 * ngpu1 * bs2 * base_lr_4.5e-6
