...
python scripts/generate_llama_mask/gen_mask_dataset.py --config scripts/generate_llama_mask/data_gen_configs/random_medium_512.yaml --indir data/coco_images/ --outdir data/myimages/inpainting/val --ext jpg
python scripts/generate_llama_mask/generate_csv.py --llama_masked_outdir data/myimages/inpainting/val/ --csv_out_path data/myimages/inpainting/val.csv
'''
import os, glob
import numpy as np
import torch
...
opt.base=['configs/latent-diffusion/inpainting/inpainting_example_overfit.yaml']
opt.finetune=''
opt.scale_lr=True
...
# model
# model = getattr( importlib.import_module('ldm.models.diffusion.ddpm', package=None) , 'LatentInpaintDiffusion' )( **config.model.get("params", dict()) ) #也可以写死这样 

from ldm.models.diffusion.ddpm import  LatentInpaintDiffusion
model = LatentInpaintDiffusion( **config.model.get("params", dict()) ) #也可以写死这样    
